odoo.define('account_dynamic_dl.Dynamic_Dl_Base', function(require) {
    'use strict';
    ...
    var DLDynamicMain = AbstractAction.extend({
        template: 'DLDynamicMain',
    ...
    var UserFilters = Widget.extend({
        ...
        start: function() {
            ...
            self.$el.append(QWeb.render('DLCompanyDatefilterLine'));

            // Getting date filters
            // Add filter type section
            self.$el.find('.date-filters').append(QWeb.render('DLDatefilterSelectionLine'));
            ...
            rpc.query({
                ...
                self.$el.find('.multi-companies').append(QWeb.render('DLMultiCompanies', { 'companies': company_ids }));
            ...
            // Date from and To
            self.$el.append(QWeb.render('DLDateLine'));

            // No need to fetch from DB. Just templates
            self.$el.append(QWeb.render('DLTargetAccountsLine'));
            //self.$el.append(QWeb.render('DLSortByInitialBalanceLine'));
            ...
            rpc.query({
                ...
                _(results).each(function(item) {
                        partners.push({ 'name': item.display_name, 'id': item.id })
                    }) //each
                self.$el.append(QWeb.render('DLPartnersLine', { 'partners': partners }));
            ...
            rpc.query({
                ...
                self.$el.append(QWeb.render('DLJournalsLine', { 'journals': journals }));
            ...
            self.$el.append(QWeb.render('DLAccount'));
            ...
            rpc.query({
                ...
                self.$el.find('.normal_acc').append(QWeb.render('DLAccountsLine', { 'accounts': accounts }));
            ...
            rpc.query({
                ...
                self.$el.find('.normal_acc_tags').append(QWeb.render('DLAccountTagsLine', { 'acc_tags': account_tags }));
            ...
            self.$el.append(QWeb.render('DLAnalytic'));
            ...
            rpc.query({
                ...
                self.$el.find('.analytic_acc').append(QWeb.render('DLAnalyticAccountLine', { 'analytic_accs': analytic_accounts }));
    ...
    var ControlButtons = Widget.extend({
        ...
        download_pdf: function(event) {
            ...
            return rpc.query({
                args: [filter, 'pdf'],
                model: 'report.account.dynamic.report_dailyledger',
                method: 'render_html',
            }).then(function(result) {
                var action = {
                    'type': 'ir.actions.report',
                    'report_type': 'qweb-pdf',
                    'report_name': 'accounting_pdf_reports.report_dailyledger',
                    'report_file': 'accounting_pdf_reports.report_dailyledger',
                    'data': result,
                    'context': {
                        'active_model': 'account.report.daily.ledger',
                        'landscape': 1,
                        'data': result
                    },
        ...
        download_xlsx: function(event) {
            ...
            return rpc.query({
                ...
                var action = {
                    ...
                    'model': 'report.account_dynamic_dl.daily_ledger_xlsx',
                    'report_type': 'xlsx',
                    'report_name': 'account_dynamic_dl.daily_ledger_xlsx',
                    'report_file': 'account_dynamic_dl.daily_ledger_xlsx',
                    'data': result,
                    'context': {
                        'active_model': 'account.report.daily.ledger',
                        'data': result
    ...
    var AccountContents = Widget.extend({
        ...
        view_invoice: function(event) {
            var self = this;
            var context = {};
            var rf = $(event.currentTarget).data('lref');
            // console.log(rf);
    ...
    core.action_registry.add('dynamic_dl_report', DLDynamicMain);
});