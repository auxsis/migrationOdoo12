...
class account_bank_statement_wizard(models.TransientModel):
    ...
    bank_opt = fields.Selection([('santander','Santander'),('estado','Banco Estado'),('chile','Banco de Chile'),('itau','Banco Itau'),('bci','Banco BCI'),('scotiabank','Scotiabank'),('santanderusd','Santander USD')])
    ...
    def import_file(self):
        ...
        if self.file_opt == 'csv':
            ...
            elif self.bank_opt == 'santanderusd':
                for row_no in range(sheet.nrows):
                    line = list(map(lambda row:isinstance(row.value, str) and row.value.encode('utf-8') or str(row.value), sheet.row(row_no)))
                    try:
                        fecha=line[2].decode("utf-8")
                ...
                for row_no in range(sheet.nrows):
                    ...
                    try:
                        ...
                        values.update( {'date':date_string,
                                        ...
                                        'partner': '',
                                        'memo': line[1].decode("utf-8"),
                                        'amount': line[4]+line[5],
                                        'company_id':self.company_id.id,
                                        'journal_id':self.journal_id.id,
                                       })
                        res = self._create_statement_lines(values)
                    except Exception as e:
                        _logger.warning('No encuentra Fecha %s',str(e))                        
                    ...
                    if row_no <= 1:
                        ...
                        if line[4]=='0': 
                            values.update( {'date':date_string,
                                            ...
                                            'amount': int(line[5].decode('utf-8').replace('.',''))* (1),
                                            'company_id':self.company_id.id,
                                            'journal_id':self.journal_id.id,                                            
                                           })
                        else: 
                            values.update( {'date':date_string,
                                            ...
                                            'partner': 'X',
                                            'memo': line[2].decode('utf-8'),
                                            'amount': int(line[6].decode('utf-8').replace('.',''))* (1),
                                            'company_id':self.company_id.id,
                                            'journal_id':self.journal_id.id,
                                           })
                        res = self._create_statement_lines(values)                        
...
#
    ...
    def _create_statement_lines(self,val):
        ...
        aaa = self._cr.execute("insert into account_bank_statement_line (date,ref,partner_id,name,amount,statement_id,company_id,journal_id) values (%s,%s,%s,%s,%s,%s,%s,%s)",(val.get('date'),val.get('ref'), partner_id,val.get('memo'),val.get('amount'),self._context.get('active_id'),val.get('company_id'),val.get('journal_id')))
        return True
#
