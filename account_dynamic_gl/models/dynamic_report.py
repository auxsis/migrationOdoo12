...
class ReportDynamicGeneralLedger(models.AbstractModel):
    ...
    def _get_account_move_entry(self, accounts, init_balance, sortby, display_account):
        ...
        if init_balance:
            ...
            sql = ("""SELECT 0 AS lid, l.account_id AS account_id, '' AS ldate, '' AS lcode,'' as lcenter,'' as lsector, NULL AS amount_currency, '' AS lref, 'Initial Balance' AS lname, COALESCE(SUM(l.debit),0.0) AS debit, COALESCE(SUM(l.credit),0.0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) as balance, '' AS lpartner_id,\
                '' AS move_name, '' AS mmove_id, '' AS currency_code, afr.name AS reconciled,\
                NULL AS currency_id,\
            ...
            LEFT JOIN account_full_reconcile afr on afr.id=l.full_reconcile_id\
                WHERE l.account_id IN %s""" + filters + ' GROUP BY l.account_id,afr.name')
        ...
        _logger.info("wheres=%s,%s",wheres, where_params)
        filters = " AND ".join(wheres)
        filters = filters.replace('account_move_line__move_id', 'm').replace('account_move_line', 'l')

        # Get move lines base on sql query and Calculate the total balance of move lines
        sql = ('''SELECT l.id AS lid, l.account_id AS account_id, COALESCE(to_char(l.date,'DD-MM-YYYY')) AS ldate, j.code AS lcode,asec.name as lsector,acen.name as lcenter, l.currency_id, l.amount_currency, l.ref AS lref, l.name AS lname, COALESCE(l.debit,0) AS debit, COALESCE(l.credit,0) AS credit, COALESCE(SUM(l.debit),0) - COALESCE(SUM(l.credit), 0) AS balance,\
            ...
            LEFT JOIN account_cost_center acen ON (l.cost_center_id=acen.id)\
            LEFT JOIN account_full_reconcile afr on afr.id=l.full_reconcile_id\
            WHERE l.account_id IN %s ''' + filters + ''' GROUP BY l.id, l.account_id, l.date, j.code, asec.name,acen.name, l.currency_id, l.amount_currency, l.ref,afr.name,l.name, m.name, c.symbol,c.position, p.display_name ORDER BY ''' + sql_sort)
        ...
        _logger.info("sccountres=%s",account_res)
        return account_res
